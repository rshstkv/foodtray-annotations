-- Migration: Fix trigger and add metadata
-- Description: Add metadata to work_items, fix occlusion_metadata and quantity in trigger

-- 1. Добавить metadata в work_items
ALTER TABLE work_items
ADD COLUMN IF NOT EXISTS metadata JSONB;

COMMENT ON COLUMN work_items.metadata IS 'Дополнительные данные: color для BUZZER, label для кастомных';

-- 2. Исправить триггер initialize_work_session
...
BEGIN
  ...
  SELECT 
    ...
    NULL::jsonb  -- НЕ копировать metadata (там данные Qwen, не нужны для work)
