-- Migration: Fix ambiguous column reference in acquire_recognition_with_steps
-- Description: Add table prefix to recognition_id in DELETE statement
...
BEGIN
  ...
  -- Используем полное имя столбца с префиксом таблицы
  DELETE FROM validation_work_log w
...
COMMENT ON FUNCTION acquire_recognition_with_steps IS 
  'Atomically acquire recognition with all validation types as steps. Deletes old in_progress work_logs to prevent duplicates.';
