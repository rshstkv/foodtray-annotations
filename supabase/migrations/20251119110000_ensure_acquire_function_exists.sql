-- =====================================================
-- Ensure acquire_recognition_with_steps function exists
-- =====================================================
-- Цель: Пересоздать функцию с правильной версией
-- Это финальная версия которая должна работать
...
BEGIN
  ...
  IF v_steps IS NULL OR jsonb_array_length(v_steps) = 0 THEN
    RAISE EXCEPTION 'No active validation types configured';
  END IF;
...
COMMENT ON FUNCTION acquire_recognition_with_steps IS 
  'Atomically acquire recognition with all validation types as steps. Ignores completed and recent in_progress work_logs.';
