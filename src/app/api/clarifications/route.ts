...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedData = data?.map(item => ({
      ...
      device_canteen_name: item.orders?.[0]?.device_canteen_name,
      pos_transaction_id: item.orders?.[0]?.pos_transaction_id,
      start_dtts: item.orders?.[0]?.start_dtts,
      rectangle: item.rectangle,
      clarification_type: item.clarification_type,
      image_url_main: item.orders?.[0]?.image_url_main,
      image_url_qualifying: item.orders?.[0]?.image_url_qualifying,
      ean_matched: item.ean_matched,
      product_name: item.product_name,
      sign: item.orders?.[0]?.sign,
