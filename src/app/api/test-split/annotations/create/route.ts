...
/**
 * POST /api/test-split/annotations/create
 * Create a new test split work annotation
 */
export async function POST(request: Request) {
  try {
    ...
    if (createError || !annotation) {
      console.error('[test-split/annotations/create] Error:', createError)
      return apiError('Failed to create annotation', 500, ApiErrorCode.INTERNAL_ERROR)
    }

    return apiSuccess({ annotation })
  } catch (error) {
    console.error('[test-split/annotations/create] Error:', error)
